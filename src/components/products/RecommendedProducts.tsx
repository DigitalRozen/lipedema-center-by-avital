...
import { trackAffiliateClick } from '@/lib/analytics/analyticsService'
...
export function RecommendedProducts({ postTags, monetizationStrategy, postId }: RecommendedProductsProps) {
  ...
  const handleAffiliateClick = async (product: Product) => {
    try {
      ...
      // Don't block the click if analytics fails
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      if (data) {
        ...
        // For 'High Ticket (Clinic Lead)', we don't filter by type (show all matching products)

        // Use matching engine to get filtered and sorted products
        const matches = matchProductsToPost(postTags, data, productType)
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  {/* Show "מומלץ עבורך" label for Digital products that match tags */}
